...
public class CounterAspect {
    ...
    @Pointcut("(execution(* com.svirski.spring.core.services.EventService.getEvent(String, ..)) && args(eventName, ..)) || "
              + "execution(* com.svirski.spring.core.services.EventService.getByName(String)) && args(eventName))")
    private void accessedByName(String eventName) {
    }

    @Pointcut("(execution(* com.svirski.spring.core.services.BookingService.getTicketPrice(String, ..)) && args(eventName,..))")
    private void getPriceByName(String eventName) {
    }

    @Pointcut("(execution(* com.svirski.spring.core.services.BookingService.bookTicket(*, com.svirski.spring.core.models.Ticket)) && args(*, ticket))")
